//AnimalController_SS.cs
//Original Author: Bit Gamey
...
//Changes: Removed custome namespace, required object to be grounded to move
//Original file under Assets\Easy Primitive Animals\FarmEdition\Scripts

using System.Collections;
using UnityEngine;
public class AnimalController_SS : MonoBehaviour
{
    //change by CZ
    [SerializeField] private bool isChicken = false;
    //end of change by CZ
    ...
    public float relativeLegSpeed = 40f;
    ...
    private void Start()
    {
        ...
        if (!isChicken)
        {
            ...
            rotSpeed = movSpeed * relativeLegSpeed; // Set legs to move relative to animal moving speed.
        }
    }
    ...
    {
        //change by CZ
        //do not move if not grounded
        if (!myParentWeighable.IsGrounded) return;
        //end of change by CZ

        if (!isChicken) // If animal the script is attached to is not a chicken, animate 4 legs and body.
        {
